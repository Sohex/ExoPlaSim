...
!              vdaero; the model configuration carries it and its bracket.
...
!     the model configuration. None has a Fortran default, for the reason
...
!        than carry a plausible-looking default that nothing in the caller
!        decided. Both values live in the model configuration.
!
         if (ldepvel == 1 .and. vdaero <= 0.0) then
            ...
            write(nud,*) '* block of the model configuration.'
         ...
         if (lwetdep == 1 .and. (scava <= 0.0 .or. scavb <= 0.0)) then
            ...
            write(nud,*) '* the model configuration, which also carries the'
...
!        for the same reason: the model configuration owns every one of these
...
!
     ...
     &          dustwcv  <= 0.0) then
               ...
               write(nud,*) '* All of them live in the model configuration'
...
!        optical effective radius of the modelled dust and 1/1948 at the
...
!     every rank has to agree about whether it is called at all.
      ...
      if (zmax <= 0.0) then
         if (mypid == NROOT) then
            ...
            write(nud,*) 'an offline preprocessing step.'
